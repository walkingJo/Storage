...
//		else if (drawType == DRAW_TYPE.AREA9)		area.drawImg(img);
	...
	private void extractor(Graphics graphics) {
		...
		if(drawType == DRAW_TYPE.PENCIL)			그림판app.setShortcutButtonOn( 4);
		else if (drawType == DRAW_TYPE.FILL)		그림판app.setShortcutButtonOn( 5);
		else if (drawType == DRAW_TYPE.ERASE)		그림판app.setShortcutButtonOn( 6);
		else if (drawType == DRAW_TYPE.EXTRACTOR)	그림판app.setShortcutButtonOn( 7);
		else if (drawType == DRAW_TYPE.LINE)		그림판app.setShortcutButtonOn( 8);
		else if (drawType == DRAW_TYPE.CURVE0)		그림판app.setShortcutButtonOn( 9);
		else if (drawType == DRAW_TYPE.TRIANGLE)	그림판app.setShortcutButtonOn(10);
		else if (drawType == DRAW_TYPE.RECTANGLE)	그림판app.setShortcutButtonOn(11);
		else if (drawType == DRAW_TYPE.OVAL)		그림판app.setShortcutButtonOn(12);
	} // 색 추출
	private void drawLine(Graphics graphics) {
		line(graphics, srcPt, dstPt);
	}
	private float curveSpeed(float t) {
//		srcPt
//		curvePt1
//		curvePt2
//		dstPt
		float speed_x =
				3 * (-srcPt.x + curvePt1.x - curvePt2.x + dstPt.x) * t * t
				+ 2 * (3 * srcPt.x - 2 * curvePt1.x + curvePt2.x) * t
				+ 1 * (-3 * srcPt.x + curvePt1.x);
		float speed_y =
				3 * (-srcPt.y + curvePt1.y - curvePt2.y + dstPt.y) * t * t
				+ 2 * (3 * srcPt.y - 2 * curvePt1.y + curvePt2.y) * t
				+ 1 * (-3 * srcPt.y + curvePt1.y);
		//3(-1+2-3+4)t^2 + 2(3*1-2*2+3)t + (-3*1+2)
